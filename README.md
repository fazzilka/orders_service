...
Это асинхронный сервис заказов на FastAPI с PostgreSQL, Redis, RabbitMQ и
  Celery: он хранит пользователей и заказы, кэширует чтение заказов и отправляет
  события о новых заказах в очередь. Пользователь может зарегистрироваться,
  получить JWT-токены, работать через Bearer или cookie-авторизацию, обновлять
  access token по refresh-cookie и выходить из системы. Основные API-функции
  сервиса — создать заказ, получить заказ по id, изменить его статус и получить
  список заказов текущего пользователя, при этом доступ к чужим заказам запрещён.
  После создания заказа сервис кладёт событие new_order в RabbitMQ, consumer
  читает его и ставит фоновую задачу в Celery, а отдельный worker имитирует
  обработку заказа.
